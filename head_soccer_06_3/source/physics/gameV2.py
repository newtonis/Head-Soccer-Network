...
class AnimationLetter:
    def __init__(self,letter,final_position,actual_position,loops,color=(100,100,200),font="Classic"):
        ...
        self.ux , self.uy = self.x,self.y #last x, last y
    ...
    def GraphicUpdate(self,screen):
        ...
        """
        self.parent.AddUpdateRect( (self.ux,self.uy,self.surface.get_size()[0],self.surface.get_size()[1]) )
        self.parent.AddUpdateRect( (self.x,self.y,self.surface.get_size()[0],self.surface.get_size()[1]) )
    ...
    def LogicUpdate(self):
        ...
        #if self.x != self.ux or self.y != self.uy:


        self.ux , self.uy = self.x , self.y
...
class LettersAnimation:
    def __init__(self,msj,color,parent,duration,font):
        ...
        for letter in msj: # Separating letters in an array
            letter = AnimationLetter(letter,(0,0),(0,0),self.loops,color,font)
            letter.SetParent(self)
            self.letters.append(letter) # Generating render of letter and inserting into array
    ...
    def AddUpdateRect(self,rect):
        if self.parent:
            self.parent.AddUpdateRect(rect)
...
class LetterSystem:
    ...
    def AddWord(self,word,color,time,font,autoRemoveTime=3):
        ...
        self.CurrentAnimation.SetParent(self)
...
class ScoreSystem(container.Container):
    ...
    def GraphicUpdate(self,screen):
        self.UpdateRects("Score A")
        self.UpdateRects("Score B")
        container.Container.GraphicUpdate(self,screen) # Graphic updating all container objects
    def UpdateRects(self,key):
        if self.Ereferences.has_key(key):
            element = self.Ereferences[key]
            self.parent.AddUpdateRect((element.x,element.y,element.image.get_size()[0],element.image.get_size()[1]))
...
class ClockSystem:
    ...
    def GraphicUpdate(self,screen):
        ...
        if not self.surface:
            return
        self.parent.AddUpdateRect((self.surface.x,self.surface.y,self.surface.image.get_size()[0],self.surface.image.get_size()[1]))
...
class PowerGameEngine(ScoreSystemCont,ClockSystemCont):
    ...
    def UpdateSequence(self):
        ...
        if not self.target:
            ...
            elif self.extraDraw.has_key("sequence"):
                self.AddUpdateRect((self.pixelWidth/2-self.extraDraw["sequence"].image.get_size()[0]/2,200 , self.extraDraw["sequence"].image.get_size()[0] , self.extraDraw["sequence"].image.get_size()[1]))
    ...
    def UpdateSequenceSurface(self):
        ...
        self.AddUpdateRect((self.pixelWidth/2-surface.get_size()[0]/2,200 , self.extraDraw["sequence"].image.get_size()[0] , self.extraDraw["sequence"].image.get_size()[1]))
    def StartGame(self):
    ...
    def StartGoalAnimation(self):
        ...
        self.extraDraw["GOAL"].SetParent(self)
    ...
    def RemoveTeamAName(self):
        ...
        if self.target:
            ...
            self.AddUpdateRect((0,30,self.pixelWidth,self.extraDraw["teamA"].image.get_size()[1]))
    ...
    def RemoveTeamBName(self):
        ...
        if self.target:
            ...
            self.AddUpdateRect((0,30,self.pixelWidth,self.extraDraw["teamB"].image.get_size()[1]))
    ...
    def GenerateAteamSurface(self):
        ...
        self.AddUpdateRect( (self.pixelWidth/2-200-text.get_size()[0],30 , self.extraDraw["teamA"].image.get_size()[0] , self.extraDraw["teamA"].image.get_size()[1] ) )
    def GenerateBteamSurface(self):
        ...
        self.AddUpdateRect( (self.pixelWidth/2+200,30 , self.extraDraw["teamB"].image.get_size()[0] , self.extraDraw["teamB"].image.get_size()[1] ) )
    def SetMaxPlayers(self,maxA,maxB):
        ...
        if self.target:
            ...
            self.UpdateRect("maxA")
            self.UpdateRect("maxB")
    ...
    def UpdateRect(self,key):
        if self.extraDraw.has_key(key):
            element = self.extraDraw[key]
            self.AddUpdateRect((element.x,element.y,element.image.get_size()[0],element.image.get_size()[1]))
    def MaxData(self):
