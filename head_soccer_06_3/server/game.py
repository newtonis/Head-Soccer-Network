...
class Game:
    ...
    def SendAllData(self,player):
        player.Send({"action":"initial_join","data_join":self.GetJoinInfo(),"pitch_data":self.PitchData(),"id":player.id,"name":player.name})
    ...
    def PlayerData(self,player,type,data=None):
        if type == "exit":
            ...
            self.PostChat(data["From"],data["Message"],(44, 132, 55))
    def PostChat(self, writer , content , color):
        self.SendToAll({"action":"sa","type":"NewChat","from":writer,"message":content,"color":color})
    ...
    def Alose(self):
        ...
        self.PostChat("Host","W.O. "+self.gameEngine.BteamName+" wins",(37, 22, 160))
    ...
    def Blose(self):
        ...
        self.PostChat("Host","W.O. "+self.gameEngine.AteamName+" wins",(37, 22, 160))
        self.gameEngine.StopClock(True)
        self.gameEngine.DeleteElement(self.ballID)
    ...
    def SendToAllUDPExcept(self,client_id,data,sendCC=False):
        for spectator in self.spectators.keys():
            if spectator != client_id:
                if sendCC:
                    data["cc"] = self.spectators[spectator].GetCC()
                self.spectators[spectator].SendUDP(data)
    ...
    def HandleUpdateMovement(self):
        ...
        self.SendToAllUDPExcept(-1,{"action":"sa","type":"multiple-me","mes":data_to_all},True)
    ##### GAME TARGET DEF #####
    ...
    def HandleStartGame(self):
        self.PostChat("Host","Starting game...",(37, 22, 160))
    ...
    def HandleEndMatch(self,textWin):
        self.PostChat("Host",textWin,(37, 22, 160))
